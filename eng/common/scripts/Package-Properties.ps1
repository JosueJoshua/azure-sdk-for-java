...
function Get-PkgProperties
{
    ...
    {
        if(($pkgProp.Name -eq $PackageOrArtifactName) -or ($pkgProp.ArtifactName -eq $PackageOrArtifactName))
        {
            return $pkgProp
        }
    }

    LogError "Failed to retrive Properties for [ $PackageOrArtifactName ]"
    return $null
}
